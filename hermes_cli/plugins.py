...
class PluginContext:
    ...
    def register_message_gate(self, stage: str, callback: Callable) -> PluginRegistration:
        """Register an async required gate, selected by this plugin's id in message_gates.

        Unlike observer hooks, errors and invalid decisions fail closed. See agent.message_gates.
        """
        from agent.message_gates import STAGES
        import inspect
        if stage not in STAGES or not inspect.iscoroutinefunction(callback):
            raise ValueError("Message gates require a supported stage and async callback")
        key = (stage, self.plugin_id)
        registry = self._manager._message_gates
        if key in registry:
            raise ValueError(f"Duplicate message gate: {key}")
        registry[key] = callback

        def release():
            if registry.get(key) is callback:
                registry.pop(key)

        return self._track("message_gate", f"{stage}/{self.plugin_id}", release)
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        self._message_gates: Dict[Tuple[str, str], Callable] = {}
